...
class ReactTags extends Component {
  ...
  filteredSuggestions(query, suggestions) {
    ...
    return suggestions.filter((key) => AutoComplete.fuzzyFilter(query, key));
  }

  componentWillReceiveProps(props) {
    const suggestions =
      props.disableFiltering? props.suggestions:this.filteredSuggestions(this.state.query, props.suggestions);
    this.setState({
  ...
  handleChange(query, dataSource, {source}) {
    if (source === 'touchTap') {
      this.setState({query: ""});
      return;
    }
    ...
    const suggestions =
      this.props.disableFiltering ? this.props.suggestions:this.filteredSuggestions(query, this.props.suggestions);
  ...
  addTag(tag) {
    ...
    const possibleMatches =
      this.props.disableFiltering? this.props.suggestions:this.filteredSuggestions(tag, this.props.suggestions);
    ...
    if (!this.props.allowCreate && selectedIndex === -1 && !possibleMatches.length) {
      return;
    }
  ...
  handleSuggestionClick(i, e) {
    this.props.handleAddition(this.state.suggestions[i]);
  }
    ...
    const tagInput =  !this.props.readOnly ? (
      <div className={this.state.classNames.tagInput}>
        <AutoComplete
            ...
            filter={this.props.disableFiltering? (() => true):AutoComplete.fuzzyFilter}
            openOnFocus={true}
            dataSource={suggestions}
            dataSourceConfig={{text: this.props.labelField  , value: 'id'}}
            onNewRequest={
              (t,i) => (i < 0)? this.handleKeyDown({
                keyCode: Keys.ENTER,
                preventDefault: () => {}
              }): this.handleSuggestionClick(i)
            }
...
export const WithContext = DragDropContext(HTML5Backend)(ReactTags);
export const WithOutContext = ReactTags;
